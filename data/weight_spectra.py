...
ir_files = os.listdir(os.path.join(folder, "new_ir", "conformer_data"))
vcd_files = os.listdir(os.path.join(folder, "new_vcd", "conformer_data"))
...
vcd_xs = []
with open(os.path.join(folder, "new_vcd/conformer_data", vcd_files[0]), "r") as infile:
...
for boltzmann_factor, vcd_file in zip(boltzmann_factors, vcd_files):
    with open(os.path.join(folder, "new_vcd/conformer_data", vcd_file), "r") as infile:
...
ir_xs = []
with open(os.path.join(folder, "new_ir/conformer_data", ir_files[0]), "r") as infile:
    for i, line in enumerate(infile):
        data = line.split()
        if i == 0:
            continue
...
for boltzmann_factor, ir_file in zip(boltzmann_factors, ir_files):
    with open(os.path.join(folder, "new_ir/conformer_data", ir_file), "r") as infile:
        for i, line in enumerate(infile, start=-1):
            data = line.split()
            if i == -1:
                continue
